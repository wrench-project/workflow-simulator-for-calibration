...
def main():
	...
	for workflow_name in workflow_names:
		for architecture in architectures:
			...
			for num_tasks in num_tasks_values:
				for cpu in cpu_values:
					for data in data_values:
						...
						key = f"{workflow_name} on {architecture} with {num_tasks} tasks {cpu} cpu and {data} data"
			...
			ax.set_xticklabels(makespans_data.keys(), rotation=45, ha='right')
			ax.set_ylabel('Makespans')
			ax.set_title('Makespans per Workflow and Architecture')

			plt.tight_layout()
			plt.savefig(f"{workflow_name}_on_{architecture}.png")
			plt.close()
