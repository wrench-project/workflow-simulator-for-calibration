...
for workflow in $WORKFLOWS; do
        for architecture in $ARCHITECTURES; do
            echo "* Experiment: $workflow on $architecture"
	        	echo python3 ./run_single_workflow_experiments.py -wd ../../JSONS -wn $workflow -ar $architecture -al $ALGORITHM -tl $TIME_LIMIT -th $NUM_THREADS -lf relative_average_error -cs htcondor_bare_metal -ss submit_and_compute_hosts  -ns one_and_then_many_links -cn CCskylake
        done 
done
