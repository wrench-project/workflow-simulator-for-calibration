...
def vary_larger_tasks_fix_arch_wf_nodes(calibrate_list, simulate_list, dir_wf, config_json, outfile, num_iter=500, timeout=60, keep=False, debug=False):
        ...
        for name in wf_names:   #for each workflow
            ...
            for node in wf_nodes:  #for each node
                ...
                for task in wf_tasks:  #for each task
                    ...
                    task_list = wfq.filter_tasks_leq(node_list, task)
                    task_list.sort()
...
def main():
    ...
    # Run vary_larger_tasks_fix_arch_wf_nodes experiment
    vary_larger_tasks_fix_arch_wf_nodes(calibrate_list, simulate_list, config['input_dir'], config['config_json'], config["output_file"], config['num_iter'], config['timeout'], config['keep_exp_dir'])
